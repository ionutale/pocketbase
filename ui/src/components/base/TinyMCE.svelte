...
<script>
    ...
    let id = "tinymce_svelte" + CommonHelper.randomString(7);
    let inline = undefined;
    let disabled = false;
    let scriptSrc = `${import.meta.env.BASE_URL}libs/tinymce/tinymce.min.js`;
    let conf = {};
    let modelEvents = "change input undo redo";
    let value = "";
    let text = "";
    let cssClass = "tinymce-wrapper";
