...
func (api *fileApi) download(e *core.RequestEvent) error {
	...
	if thumbSize == "" {
		thumbSize = e.Request.URL.Query().Get("size")
		useSizeParam = true
	}
	...
	if thumbSize != "" {
		if useSizeParam {
			isValidThumb = filesystem.ThumbSizeRegex.MatchString(thumbSize)
		} else {
			isValidThumb = list.ExistInSlice(thumbSize, defaultThumbSizes) || list.ExistInSlice(thumbSize, fileField.Thumbs)
		}
	}
